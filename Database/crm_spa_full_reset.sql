...
CREATE DATABASE crm_spa CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE crm_spa;

-- Drop all tables if they exist (in correct dependency order)
DROP TABLE IF EXISTS CostOnOrder;
DROP TABLE IF EXISTS FeedbackOnOrders;
DROP TABLE IF EXISTS AppInfoCheck;
DROP TABLE IF EXISTS PrepaidCardTransaction;
...
-- Create tables (same structure as before, with datatype fix: User.ID is BIGINT)
-- [...] Truncated to save space here, but full content will be written to file
...
CREATE TABLE `Order` (
    ...
    ActualRevenue DECIMAL(18,2) DEFAULT 0,
    PaymentMethodID INT,
